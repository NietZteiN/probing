...
| id | experiment | serves | command | where | cost | status |
|---|---|---|---|---|---|---|
| E12 | Irrelevant-lure control at level 4: number word on the distractor; lure rate and probe lure mass vs incongruent@v2 | "binding vs presence" sentence in RQ1 | `pipeline.py --stage all --levels 4` | h200 | 3 GPU-h per model | **behaviour done both models**: irrelevant lure rate 1.8% (3B) / 4.9% (8B, direct) vs baselines ~1.1 / ~3; presence without binding costs accuracy (3B chain 76.4 vs 81.5) but yields no lure answers. 3B level-4 probes submitted |
| E13 | Levels 2 (in order) and 5 (three steps): behaviour + probes | robustness of C1–C3 across dependency structure and depth | `pipeline.py --stage run,probes --levels 2 5` | h200 | 4 GPU-h per model | **behaviour done both models** (results/NOTES.md 2026-09-13); 3B level-2 probes submitted |
...
| E33 | Demonstration-artefact check: rerun level-3 behaviour with a second fixed demo set (seed 11; regimes `cot_s11` / `direct_s11`) for both core models; the level-3 seed-7 demos contain one instance where the intermediate's value equals the first equation's operand, which is the 8B model's dominant level-3 error. If the copy error vanishes, the level-3 absolute accuracies and the size of the congruent-name effect are re-based on the seed-11 run; if it persists, it is a model property | validity of Table 1 absolute numbers; E32 | `20_run_model.py --regimes cot_s11 direct_s11 --no-train` | h200 | 0.5 GPU-h per model | submitted 2026-09-13 |
