...
| E33 | Demonstration-artefact check: rerun level-3 behaviour with a second fixed demo set (seed 11; regimes `cot_s11` / `direct_s11`) for both core models; the level-3 seed-7 demos contain one instance where the intermediate's value equals the first equation's operand, which is the 8B model's dominant level-3 error. If the copy error vanishes, the level-3 absolute accuracies and the size of the congruent-name effect are re-based on the seed-11 run; if it persists, it is a model property | validity of Table 1 absolute numbers; E32 | `20_run_model.py --regimes cot_s11 direct_s11 --no-train` | h200 | 0.5 GPU-h per model | first attempt (392678/392679) died with ENOSPC at the ~3 TB per-user scratch limit; resubmit after freeing the *_alt caches |
