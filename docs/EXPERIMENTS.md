...
| E8 | Patching `main`: neutral → incongruent at every token carrying the target name; single layers, 4-layer windows, all layers; read next token at `cotpre@r` and `anspre`; recovery = P(correct after \| lure error before) | C4 → Figure 3 | `pipeline.py --stage patch` | h200 | 1.5 GPU-h per (model, regime) | **main done** for llama32-3b: direct-regime lure removal 52%/39% at all layers, 75%/46% at layer 1; CoT has no lure errors. Controls rerunning (391922/391923); 8B queued |
