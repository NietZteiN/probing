...
with Levels 2 and 5 as robustness checks and Level~4 for a control described below.
...
never correct for another reason. Neutral words are matched to number words in token count
under each model's tokenizer, checked per model, so positions align across twins and length is
