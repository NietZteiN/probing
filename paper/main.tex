...
\citeauthor{kudo2026faithful}'s panel, plus further families and levels in the appendix. Behavioural metrics are
...
\caption{Behaviour by model, level and regime. $S$ is the number of demonstration sets;
Neutral and Letter are mean accuracy (\%) with the across-set range. Facilitation is congruent
minus neutral, interference is neutral minus incongruent, and Lure$-$base is the rate of
answers equal to the lure minus the rate at which the neutral twin gives that same digit.
$^{*}$ marks a contrast whose sign holds in every demonstration set and whose pooled
cluster-bootstrap interval excludes zero. Deeper variables at Level~5 are in the appendix.}
\label{tab:behavior}
\end{table}
...
Levels 2, 4 and 5 and both models are reported for every behavioural quantity across three
demonstration sets, with the per-set values, the pooled estimate and the claim rule of our
pre-registration. The appendix also contains the span-by-layer patching grids for all three
