...
def main() -> int:
    ...
    if not a.no_tokenizer:
        ...
        for iid in pick:
            ...
            # tokenize_layout already slices from the instance start, so no further offset
            ts = tt["token_strings"][:n_pos]
            if len(ts) != n_pos:
                raise RuntimeError(f"{iid}: {len(ts)} token strings for {n_pos} positions")
            per_tokens[iid] = ts

    # one file per instance: a multi-panel sheet has to be shrunk to fit a page, which is what
    # made the digits unreadable. Each sheet is authored at its printed size instead.
    made = []
    ...
    # only carries information where it differs from this.
      ...
      for li in range(B.shape[0]):
          for t in range(B.shape[1]):
              axb.text(t, li, str(B[li, t]), ha="center", va="center", fontsize=6.2, color="0.35")
      ...
      for ax, iid in zip(axes[1:, 0], [iid_only]):
          ...
          for li in range(P.shape[0]):
              for t in range(P.shape[1]):
                  ax.text(t, li, str(P[li, t]), ha="center", va="center", fontsize=6.2,
          ...
          ax.set_title(f"“{x['names'][a.role]}” holds {true_v}; the name denotes {lure_v}; model {ok}",
                       loc="left", fontsize=8.5)
          toks_i = list(per_tokens.get(iid) or mte["token_strings"][:n_pos])[:n_pos]
          assert len(toks_i) == n_pos, f"{len(toks_i)} labels for {n_pos} ticks"
          ax.set_xticks(range(n_pos))
          ax.set_xticklabels([t.replace("\n", "\\n") for t in toks_i], rotation=90,
                             family="monospace", fontsize=6.0)
      ...
      stem = FIG / f"fig5_kudo3_{a.model}_L{a.level}_{a.regime}_{a.role}_{tag}_i{fig_i}"
      fig.savefig(stem.with_suffix(".pdf"), bbox_inches="tight", pad_inches=0.12)
      fig.savefig(stem.with_suffix(".png"), dpi=150, bbox_inches="tight", pad_inches=0.12)
      plt.close(fig); made.append(stem.name)
      print(f"wrote {stem.name}  ({iid_only})")
    return 0
