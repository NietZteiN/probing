...
def main() -> int:
    ...
    out = ["% generated by scripts/66_figure_dump.py",
           "\\clearpage", "\\section{Every figure at full size}",
           "Each figure below is reproduced on its own page. Wide figures are rotated a quarter turn "
           "so that the token labels are legible in print.", ""]
    for stem in ordered:
        ...
        # a figure wider than it is tall fills more of the page rotated
        rot = w / h > 1.15
        box = ("angle=90,height=\\textwidth,width=0.93\\textheight,keepaspectratio" if rot
               else "width=\\textwidth,height=0.93\\textheight,keepaspectratio")
        out += [f"\\begin{{figure*}}[p]\\centering",
                f"\\includegraphics[{box}]{{figures/{stem}.pdf}}",
                f"\\caption{{{cap}}}\\label{{fig:dump-{stem.replace('_', '-')}}}",
                f"\\end{{figure*}}", "\\clearpage", ""]
